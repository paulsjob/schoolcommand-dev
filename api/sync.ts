...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { data: children, error: childrenErr } = await supabase.from("children").select("id");
  if (childrenErr) return res.status(500).json({ error: childrenErr.message });

  if (!children || children.length === 0) {
    const seed = await supabase.from("children").insert([
  ...
  const insItems = await supabase.from("items").insert(newItems).select("*");
  if (insItems.error) return res.status(500).json({ error: insItems.error.message });

  const briefingText =
    "Sync complete. Demo data inserted into Supabase. Next step: replace demo generation with Gemini extraction + dedupe.";

  const insBrief = await supabase.from("briefings").insert([{ text: briefingText }]).select("*");
  if (insBrief.error) return res.status(500).json({ error: insBrief.error.message });
